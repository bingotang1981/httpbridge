...
func startServer(ip string, token string) {
	http.HandleFunc("/upload", func(w http.ResponseWriter, r *http.Request) {
		...
		log.Println("Start up request: ", uuid)

		// // Check if Transfer-Encoding is chunked
		// if r.Header.Get("Transfer-Encoding") == "chunked" {
		// 	fmt.Println("Received chunked request.")
		// } else {
		// 	fmt.Println("Received non-chunked request.")
		// }
		...
		conn := &twPiece{tcpconn, uuid, false, time.Now().Unix()}
		setConn(uuid, conn)
		// defer conn.Close()
		...
		for {
			...
			if n > 0 {
				...
				_, err2 := conn.tcpConn.Write(buf[:n])
				if err2 != nil {
					log.Println("Error sending up request ", err2)
				}
			}
